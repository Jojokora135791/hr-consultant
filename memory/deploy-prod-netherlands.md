...
description: "УСТАРЕЛО (legacy). Пилот-деплой в Нидерландах (Docker, Anthropic). Проект переехал внутрь контура Контура на KonturGPT — см. [[kontur-integrations]]"
metadata:
  type: project
---

> ⚠️ **УСТАРЕЛО (02.07.2026).** Этот деплой был нужен из-за геоблока Anthropic (Claude из РФ = 403).
> В v8 диалог переехал на **KonturGPT** (внутренняя модель), n8n живёт **внутри контура Контура**
> (`n8n-common.testkontur.ru`) — Нидерланды больше не нужны. Артефакты `deploy/` оставлены как референс.
> Актуальная инфра и интеграции — [[kontur-integrations]]. Ниже — исторические заметки о пилоте.

HR-консультант был развёрнут пилотом (условные данные, не реальные ПД).
