...
> 🚦 **Статус отладки (08.07.2026):** отлажены вживую **Mattermost** (вход/выход), **переезд**
...
> ⚠️ Агент временно на **отладочном промпте (Шаги 1–3: приветствие → сценарий → идентификация)** —
> сбор фактов, сроки и генерация отрезаны, размораживаем по шагам. Не протестировано: относительные
> даты в живом агенте, генерация через Mattermost. Подробно — ROADMAP «Статус отладки».
...
⚠️ Сейчас в проде **отладочный промпт**: только Шаги 1–3 (приветствие → сценарий → идентификация),
дальше стоп «в разработке». Остальные шаги размораживаем по одному.
...
| Живой тест: относительные даты, генерация в MM | Клюев О. | Не тестировано |
| Разморозка шагов агента (сбор фактов → сроки → генерация) — сейчас отладочный промпт (Шаги 1–3) | Клюев О. | В работе |
